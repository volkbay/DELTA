...
"""
This script can be called to preprocess the SLED dataset for the DELTA model, transforming the raw
...
def compute_event_volume(events: ndarray, bins: int) -> Tensor:
  ...
  # We extract the polarities as an array of +1/-1 (instead of bools)
  pol = np.empty(events["pol"].shape, dtype=np.int32)
  pol[events["pol"]] = 1
  pol[~events["pol"]] = -1

  # We create an index of unique (x, y) events
  idx, u_evts = pd.factorize(events[["x", "y"]])

  # Then, for each bin...
  for i in range(bins):
    # We compute the sum of the pol*max(0, 1-abs(bin-t_star)) for each pixel
    sums = np.bincount(idx, pol*np.fmax(0, 1-abs(i-t_star)))

    # We set these values inside the event volume
    event_volume[i, u_evts["y"], u_evts["x"]] = sums
